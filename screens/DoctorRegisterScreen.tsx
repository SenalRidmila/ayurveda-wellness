...
const DoctorRegisterScreen: React.FC<Props> = ({ navigation }) => {
    ...
    try {
      ...
      if (exists) {
        Alert.alert(
          ...
          'This email address is already registered in our system. Please use a different email or sign in with this email.',
          [
            {
  ...
  const handleRegister = async () => {
    // Enhanced validation with email existence check
    ...
    // Final email existence check if not already checked
    if (emailExists === null) {
      setEmailCheckLoading(true);
      try {
        ...
        console.log('Final email check error:', error);
      } finally {
        setEmailCheckLoading(false);
      }
    }
    ...
    try {
      ...
      Alert.alert(
        ...
        '🚫 Login: Temporarily disabled until approved\n' +
        '✅ Next Step: Wait for admin approval\n\n' +
        'You will be able to login once our admin team approves your registration. This usually takes 1-2 business days.',
        [
          {
            text: 'Understood',
            onPress: () => navigation.navigate('DoctorLogin')
      ...
      if (error.code === 'auth/email-already-in-use') {
        ...
        Alert.alert(
          'Registration Failed', 
          `Failed to create doctor account: ${error.message || 'Please try again.'}`
        );
      }
    } finally {
      setLoading(false);
          ...
          <View style={styles.formContainer}>
              ...
              <View style={styles.emailInputContainer}>
                <TextInput
                  style={[
                    styles.input, 
                    styles.emailInput,
                    emailExists === true && styles.inputError,
                    emailExists === false && styles.inputSuccess
                  ]}
                  value={email}
            ...
            <TouchableOpacity
              style={[
                ...
                (loading || emailCheckLoading || emailExists === true) && styles.registerButtonDisabled
              ]}
              onPress={handleRegister}
              disabled={loading || emailCheckLoading || emailExists === true}
            >
              {(loading || emailCheckLoading) ? (
                <View style={styles.loadingContainer}>
                  <ActivityIndicator color="#FFFFFF" size="small" />
                  <Text style={styles.loadingText}>
                    {emailCheckLoading ? 'Checking Email...' : 'Creating Account...'}
                  </Text>
                </View>
              ) : (
